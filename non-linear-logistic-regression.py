...
#####
#
# A logistic regression on (almost) quadratically separable data, i.e. can be separated by a curve of degree 2 (namely, a circle)
#
#####
# Initialize the pseudorandom data, modelled in this case after the function y=sqrt(x1^2 + x2^2)
...
output_data = np.array([[ANN.threshold(np.linalg.norm(input_data[i,0:2]) + random.normalvariate(0,0.5), 3)] for i in range(NUMBER_OF_DATA_POINTS)])
model_data = np.array([[ANN.threshold(np.linalg.norm(input_data[i,0:2]), 3)] for i in range(NUMBER_OF_DATA_POINTS)])
...
training_cost, validation_cost, accuracy = neural_net.train_network(input_data,
                                                                    ...
                                                                    display_performance_frequency = 500,
